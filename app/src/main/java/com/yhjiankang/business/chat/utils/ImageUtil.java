package com.yhjiankang.business.chat.utils;
...
import com.yhjiankang.business.chat.utils.file.AttachmentStore;
import com.yhjiankang.business.chat.utils.file.FileUtil;
import com.yhjiankang.business.chat.utils.storage.StorageType;
import com.yhjiankang.business.chat.utils.storage.StorageUtil;
...
public class ImageUtil {
    ...
    public static Bitmap getDefaultBitmapWhenGetFail() {
        try {
            return getBitmapImmutableCopy(App.getInstance().getResources(),
    ...
    public static String makeThumbnail(Context context, File imageFile) {
        ...
        boolean result = scaleThumbnail(
                imageFile,
                thumbFile,
                (int) (165.0 / 320.0 * ScreenUtil.screenWidth),
                (int) (76.0 / 320.0 * ScreenUtil.screenWidth),
                CompressFormat.JPEG,
                60);
        if (!result) {
    ...
    public static int[] getBoundWithLength(int maxSide, Object imageObject, boolean resizeToDefault) {
        ...
        if (String.class.isInstance(imageObject)) {
            ...
            bound = BitmapDecoder.decodeBound(App.getInstance().getResources(), (Integer) imageObject);
            width = bound[0];
            height = bound[1];
    ...
    public static Bitmap getBitmapFromDrawableRes(int res) {
        try {
            return getBitmapImmutableCopy(App.getInstance().getResources(), res);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
