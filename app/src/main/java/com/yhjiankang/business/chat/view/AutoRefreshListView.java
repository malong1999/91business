package com.yhjiankang.business.chat.view;
...
public class AutoRefreshListView extends ListView {
    ...
    /**
     * 在listview的onscroll    onScrollStateChanged中调用
     *
     */
    ...
    /**
     * progressBar头部————listview头布局
     */
    private ViewGroup refreshHeader;
    /**
     * progressBar脚部————listview尾布局
     */
    ...
    /**
     * 将initRefreshListener产生的列表添加到列表中，此listener上下拉刷新相关
     * @param l
     */
     ...
     * 到达顶部onRefresh(true)    底部onRefresh(false);
     */
    private void initRefreshListener() {
        OnScrollListener listener = new OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                if (scrollState == SCROLL_STATE_IDLE && state == State.RESET) {//静止&&
